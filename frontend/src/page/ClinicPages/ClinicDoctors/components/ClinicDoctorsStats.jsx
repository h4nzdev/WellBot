...
const ClinicDoctorsStats = () => {
  ...
  const clinicDoctor = doctors?.filter((doc) => doc.clinicId?._id === user._id);
  const activeDoctors = clinicDoctor?.filter(
    (d) => d.status === "Active"
  ).length;
  return (
      ...
      <div className="bg-white rounded-2xl shadow-lg border border-slate-200 p-6 hover:shadow-xl transition-all duration-300 hover:-translate-y-1">
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-4xl font-semibold text-cyan-600">{clinicDoctor?.length}</p>
          </div>
